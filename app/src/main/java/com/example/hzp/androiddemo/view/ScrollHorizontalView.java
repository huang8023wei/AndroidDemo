...
public class ScrollHorizontalView extends ViewGroup {
    private Scroller scroller;//滑动控制
    private Context mContext;
    private VelocityTracker mVelocityTracker;//用于判断甩动手势
    private static final int SNAP_VELOCITY = 500;//x轴速度基值，大于改值进行切换
    private int mCusScreen;//当前页面为第几屏
    private float mLastMotionX;//记住上次触摸屏位置
    private int delteX;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        scrollTo(mCusScreen * width, 0);//移动到第一页位置
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        float x = event.getX();//得到初始值x
        Log.d("ScrollSmoothView", "X:" + x);
        switch (action) {
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                if (xVelocity > SNAP_VELOCITY && mCusScreen > 0) {
